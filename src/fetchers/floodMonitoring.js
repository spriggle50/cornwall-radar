...
async function getFloodAndRiverLevels({ lat = DEFAULT_LAT, lon = DEFAULT_LON } = {}) {
  ...
  // These two calls used to run one after another, so if this API was slow
  // or unreachable, a full 8s timeout on EACH meant up to 16s just for this
  // one source — and since the whole dashboard waits on its slowest source,
  // that alone could make the entire page take 16s to load. Running them
  // at the same time caps this source's worst case at 8s instead.
  const [floodsResult, stationsResult] = await Promise.allSettled([
    fetchJson(floodsUrl),
  ...
  if (floodsResult.status === 'fulfilled') {
    warnings = (floodsResult.value.items || []).map((f) => ({
    ...
    warningsError = floodsResult.reason && floodsResult.reason.message;
  }
  ...
  if (stationsResult.status === 'fulfilled') {
    try {
      const nearby = (stationsResult.value.items || []).slice(0, 8);
    ...
    stationsError = stationsResult.reason && stationsResult.reason.message;
  }
